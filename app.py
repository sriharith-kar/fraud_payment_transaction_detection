...
def predict():
    ...
    model = joblib.load('Models/model_eu.sav')
    predict = model.predict(final4)

    if predict == 0:
        output = "CREDIT CARD TRANSACTION IS LEGITIMATE!" 
    elif predict == 1:
        output = "CREDIT CARD TRANSACTION IS FRAUD!" 
...
def predict1():
    ...
    print(int_features,len(int_features))
    final4=[np.array(int_features)]
    model = joblib.load('Models/model_spark.sav')
    predict = model.predict(final4)
